...
- `Technical/Sitemap_Automatic_Generation_Rule_2026-06-28.md` — **active technical SEO rule**. `sitemap.xml` is already generated automatically from the site workflow and must continue to be treated as a generated artifact. Future chats must first identify and use the existing sitemap mechanism rather than proposing manual maintenance or a second generator.
...
```text
sitemap.xml is already generated automatically by the existing site workflow.
It must continue to be treated as a generated artifact.
Do not maintain it manually.
Do not create a second generator unless the existing mechanism is explicitly replaced.
```
