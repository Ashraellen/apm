...
Это правило важнее технического положения страницы. Архив всегда строится от самой свежей завершённой дуги к самой старой, поэтому сверху стоит дуга с самым большим историческим номером, а внизу — `ДУГА 0001`.
...
## Правило читательской навигации между дугами

Внутри одной дуги отдельные страницы мыслей читаются по возрастанию номеров:
...
Но последняя мысль дуги не должна вести прямо на отдельную первую мысль следующей по чтению дуги и не должна вести на последнюю мысль предыдущей дуги.

Правильный переход в конце дуги:

```text
последняя мысль текущей дуги
→ кнопка «Следующая дуга →»
→ страница следующей по чтению дуги целиком
```

То есть читатель после завершения дуги попадает сначала на страницу дуги, видит всю шестёрку, её заголовок, общее описание и только потом выбирает отдельную мысль.

Примеры:

```text
0018
→ Следующая дуга → /[lang]/public/thoughts/
→ ДУГА 0002: 0007–0012
...
Если более старой дуги нет, кнопку `Следующая дуга →` не добавлять; вместо этого оставить возврат к публичной витрине или архиву.
...
Не делать 0018 → 0012.
Не делать 0018 → 0007.
Правильно: 0018 → страница ДУГИ 0002.

Не делать 0012 → 0001.
Правильно: 0012 → страница ДУГИ 0001.
```
...
Даже если исторически эта дуга старше, для читателя это следующий шаг чтения архива.

## Правило карточек на страницах дуг

На страницах дуг каждая карточка отдельной опорной мысли должна иметь видимую кнопку:

```text
Открыть мысль →
```

Карточка при этом может оставаться кликабельной целиком. Видимая кнопка нужна не вместо кликабельной карточки, а как ясный визуальный указатель для читателя.

Правильный паттерн карточки:

```html
<a class="thought-card" href="arcs/0007-empty-chair.html">
  <img src="../../../assets/thoughts/0007-empty-chair.jpg" alt="Пустой стул">
  <p class="thought-kicker">Опорная мысль 0007</p>
  <h2>Пустой стул</h2>
  <p>Некоторые люди не уходят полностью. Они просто перестают сидеть рядом.</p>
  <span class="formula-open">Открыть мысль →</span>
</a>
```

Это правило действует для всех архивных страниц дуг:
...
```text
1. Единообразие с живой витриной /[lang]/public/.
2. Читателю видно, куда нажимать.
3. Карточка выглядит завершённым модулем: образ → номер → название → формула/описание → действие.
```

Не делать архивные карточки без явной кнопки, даже если вся карточка кликабельна.
...
7. на страницах дуг добавить в каждую карточку видимую кнопку `Открыть мысль →`, сохранив кликабельность всей карточки;
8. затем делать языковые проходы по EN, PL, UK, BE, PT, ES, FR, DE;
9. фиксировать каждый языковой проход в session log;
10. обновлять `Projects/Ashraellen/MainSite/INDEX.md`.
...
- главный ключ: чем больше порядковый номер дуги, тем она свежее;
- внутри дуги отдельные мысли читаются по возрастанию номеров;
- в конце дуги кнопка `Следующая дуга →` должна вести на страницу следующей по чтению дуги целиком, а не на отдельную мысль;
- пример: 0018 → /[lang]/public/thoughts/; 0012 → /[lang]/public/thoughts/index-0001.html;
- на страницах дуг каждая карточка отдельной мысли должна иметь видимую кнопку `Открыть мысль →`, при этом вся карточка может оставаться кликабельной;
